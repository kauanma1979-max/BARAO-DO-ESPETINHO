...
const Catalog: React.FC<CatalogProps> = ({ products, articles, cart, addToCart, updateCartQuantity }) => {
  ...
  return (
    <div className="space-y-6 md:space-y-10 py-6 overflow-hidden">
      {/* Category Filter */}
      <div className="flex gap-2 md:gap-4 overflow-x-auto pb-4 no-scrollbar -mx-4 px-4 sticky top-16 md:top-20 z-30 bg-slate-50/95 backdrop-blur-md pt-4">
        {categories.map(cat => (
          <button 
            key={cat}
      ...
      {activeCategory === Category.TIPS ? (
        /* Blog Display correlating with Dicas do Barão */
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 animate-fade-in-up">
          {articles.length > 0 ? (
            ...
            <div className="col-span-full py-24 text-center bg-white rounded-[3rem] border-2 border-dashed border-gray-100">
               ...
               <p className="font-black text-gray-300 uppercase tracking-widest">Nenhuma dica publicada ainda.</p>
            </div>
          )}
        </div>
      ) : (
            ...
            return (
              <div key={product.id} className="bg-white rounded-[1.5rem] md:rounded-[2.5rem] overflow-hidden shadow-sm border border-gray-100 group flex flex-col h-full transition-all hover:shadow-xl fade-in-up">
                ...
                <div className="p-2.5 md:p-5 flex flex-col flex-grow">
                  <h3 className="text-xs md:text-lg font-black text-onyx mb-1 leading-tight group-hover:text-ferrari truncate uppercase">{product.name}</h3>
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-1.5 mt-auto">
                    ...
                    {cartItem ? (
                      <div className="flex items-center gap-1 bg-slate-50 rounded-lg p-0.5">
                        <button onClick={() => updateCartQuantity(product.id, -1)} className="w-6 h-6 flex items-center justify-center rounded-md bg-white text-onyx shadow-sm">-</button>
                        <span className="font-black text-onyx text-[10px] px-1">{cartItem.quantity}</span>
                        <button onClick={() => updateCartQuantity(product.id, 1)} className="w-6 h-6 flex items-center justify-center rounded-md bg-white text-onyx shadow-sm">+</button>
                      </div>
                    ) : (
                      <button onClick={() => addToCart(product)} disabled={!hasStock} className="h-8 w-8 md:w-12 md:h-12 flex items-center justify-center rounded-lg bg-onyx text-white hover:bg-ferrari transition-all shadow-md"><i className="fas fa-basket-shopping text-[10px] md:text-base"></i></button>
                    )}
                  </div>
                </div>
              </div>
