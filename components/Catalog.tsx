...
const Catalog: React.FC<CatalogProps> = ({ products, articles, cart, addToCart, updateCartQuantity }) => {
  ...
  return (
    <div className="space-y-6 md:space-y-10 py-6 overflow-hidden">
      ...
      {/* Blog Display when Category is TIPS */}
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-1.5 mt-auto">
                    ...
                    {cartItem ? (
                      <div className="flex items-center gap-1 md:gap-2 bg-slate-50 rounded-lg md:rounded-xl p-0.5 md:p-1 border border-gray-100 shadow-inner">
                        <button onClick={() => updateCartQuantity(product.id, -1)} className="w-6 h-6 md:w-8 md:h-8 flex items-center justify-center rounded-md md:rounded-lg bg-white text-onyx shadow-sm hover:text-ferrari transition-all active:scale-90"><i className="fas fa-minus text-[6px] md:text-[10px]"></i></button>
                        <span className="font-black text-onyx min-w-[14px] md:min-w-[20px] text-center text-[10px] md:text-xs">{cartItem.quantity}</span>
                        <button onClick={() => updateCartQuantity(product.id, 1)} className="w-6 h-6 md:w-8 md:h-8 flex items-center justify-center rounded-md md:rounded-lg bg-white text-onyx shadow-sm hover:text-ferrari transition-all active:scale-90" disabled={product.stock <= cartItem.quantity}><i className="fas fa-plus text-[6px] md:text-[10px]"></i></button>
                      </div>
                    ) : (
