...
const AdminPanel: React.FC<AdminPanelProps> = ({ products, orders, setProducts, setOrders, logo, setLogo, onLogout }) => {
  ...
  // Estatísticas e Cálculo de Vendas por Produto
  const stats = useMemo(() => {
    ...
    // Mapear vendas por produto para o gráfico em lista
  ...
  // Agrupamento para o gráfico em lista por categoria
  ...
  // Agrupamento de produtos para a aba Estoque
    ...
    <div className="py-8 animate-fade-in px-4 md:px-0">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
        <div className="flex items-center gap-8">
          <div className="w-28 h-28 bg-ferrari rounded-[2.5rem] flex items-center justify-center text-white shadow-2xl shadow-ferrari/30">
          ...
          <div>
            ...
            <p className="text-gray-400 font-bold uppercase text-sm tracking-[0.5em] mt-4">Painel Administrativo Premium</p>
          </div>
        </div>
        <button onClick={onLogout} className="px-14 py-7 bg-white text-gray-500 rounded-3xl font-black uppercase text-base tracking-widest hover:text-ferrari transition-all flex items-center gap-4 shadow-sm border border-gray-100">
          <i className="fas fa-power-off text-2xl"></i> SAIR
        </button>
      </div>
      ...
      <div className="flex gap-4 overflow-x-auto no-scrollbar mb-12 -mx-4 px-4">
        {[
          ...
          <button 
            ...
            className={`flex items-center gap-6 px-12 py-8 rounded-[2.5rem] font-black uppercase text-lg tracking-[0.1em] transition-all whitespace-nowrap border-2 ${activeTab === tab.id ? 'bg-onyx text-white border-onyx shadow-2xl shadow-onyx/30 scale-105' : 'bg-white text-gray-400 border-gray-100 hover:border-gray-200'}`}
          >
            <i className={`fas ${tab.icon} text-3xl ${activeTab === tab.id ? 'text-ferrari' : 'text-gray-300'}`}></i>
            {tab.label}
          </button>
        ))}
      </div>
      ...
      <div className="space-y-16">
               ...
               {[
                 ...
                 <div key={i} className="bg-white p-12 rounded-[3.5rem] shadow-sm border border-gray-100 relative overflow-hidden group">
                   <div className={`absolute -right-10 -bottom-10 opacity-5 group-hover:scale-125 transition-transform duration-1000`}>
                     <i className={`fas ${stat.icon} text-[15rem]`}></i>
                   </div>
                   <span className="text-sm font-black uppercase text-gray-400 tracking-[0.4em] block mb-4">{stat.label}</span>
                   <span className={`text-4xl font-black text-onyx tracking-tighter block`}>{stat.val}</span>
                 </div>
               ))}
            </div>

            {/* Gráfico Moderno em Lista Separado por Categoria */}
            <div className="bg-white p-12 md:p-16 rounded-[4.5rem] shadow-sm border border-gray-100">
               ...
               <div className="space-y-16">
                 {/* Fix: Added explicit type casting to any[] for items in Object.entries(salesByCategory) map to resolve 'unknown' type error */}
                 {Object.entries(salesByCategory).map(([category, items]) => (items as any[]).length > 0 && (
                   <div key={category} className="space-y-8">
                      ...
                      <div className="grid grid-cols-1 gap-6">
                        {/* Fix: Added explicit type casting to any[] for items.map to resolve 'unknown' type error */}
        ...
        {activeTab === 'inventory' && (
          <div className="space-y-12 animate-fade-in-up">
            <div className="flex justify-between items-center bg-white p-16 rounded-[4rem] shadow-sm border border-gray-100">
              <div>
              ...
              <button 
                ...
                className="bg-ferrari text-white px-14 py-8 rounded-[2.5rem] font-black uppercase text-base tracking-[0.2em] shadow-2xl shadow-ferrari/20 hover:scale-105 transition-all flex items-center gap-6"
              >
                <i className="fas fa-plus text-3xl"></i> NOVO ITEM
              </button>
            </div>
              ...
              <div key={category} className="space-y-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                  {(productsInCategory as Product[]).map(product => (
                    <div key={product.id} className="bg-white p-12 rounded-[3.5rem] shadow-sm border border-gray-100 flex items-center gap-10 group hover:border-ferrari transition-all relative">
                      <img src={product.image} className="w-32 h-32 rounded-3xl object-cover shadow-xl group-hover:scale-110 transition-transform" />
                      <div className="flex-grow">
                        ...
                        <div className="flex items-center gap-10 pt-6 border-t border-gray-50">
                          <div className="flex flex-col">
                            <span className="text-sm font-black text-gray-300 uppercase tracking-widest mb-2">Quant.</span>
                            <span className={`text-2xl font-black ${product.stock < 10 ? 'text-ferrari' : 'text-onyx'}`}>{product.stock} un</span>
                          </div>
                          <div className="flex flex-col">
                            <span className="text-sm font-black text-gray-300 uppercase tracking-widest mb-2">Preço</span>
                      ...
                      <button 
                        ...
                        className="absolute top-8 right-8 w-16 h-16 bg-onyx text-white rounded-2xl flex items-center justify-center hover:bg-ferrari transition-all shadow-xl shadow-black/10"
                      >
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50">
                    <th className="p-14 text-sm font-black uppercase tracking-widest text-gray-400 border-b border-gray-100">IDENTIFICAÇÃO</th>
                    <th className="p-14 text-sm font-black uppercase tracking-widest text-gray-400 border-b border-gray-100">CLIENTE</th>
                    <th className="p-14 text-sm font-black uppercase tracking-widest text-gray-400 border-b border-gray-100">ITENS</th>
                    <th className="p-14 text-sm font-black uppercase tracking-widest text-gray-400 border-b border-gray-100 text-center">STATUS</th>
                  </tr>
                </thead>
                <tbody>
                  {orders.slice().reverse().map(order => (
                    <tr key={order.id} className="hover:bg-slate-50/50 transition-colors">
                      <td className="p-14 border-b border-gray-100">
                      ...
                      <td className="p-14 border-b border-gray-100">
                        ...
                        <a href={`https://wa.me/55${order.customer.phone.replace(/\D/g,'')}`} target="_blank" rel="noopener noreferrer" className="text-sm text-green-600 font-black uppercase tracking-widest flex items-center gap-4 hover:scale-105 transition-transform origin-left">
                           <i className="fab fa-whatsapp text-3xl"></i> {order.customer.phone}
                        </a>
                      </td>
                      <td className="p-14 border-b border-gray-100">
                      ...
                      <td className="p-14 border-b border-gray-100">
                        <div className="flex justify-center">
                          <select 
                            ...
                            className={`px-12 py-7 rounded-[2.5rem] font-black uppercase text-sm tracking-[0.2em] outline-none border-2 transition-all cursor-pointer shadow-sm ${
        ...
        {activeTab === 'store' && (
          <div className="bg-white rounded-[4rem] p-20 shadow-sm border border-gray-100 animate-fade-in-up max-w-4xl">
            <h3 className="text-4xl font-black uppercase tracking-tighter text-onyx mb-16 flex items-center gap-8">
              <i className="fas fa-image text-ferrari text-6xl"></i> Identidade Visual
            </h3>
            <div className="flex flex-col items-center gap-12 p-20 border-4 border-dashed border-slate-100 rounded-[4rem] bg-slate-50/50">
              {logo ? (
                <div className="relative group">
                  <img src={logo} alt="Store Logo" className="w-80 h-80 rounded-[3.5rem] object-cover shadow-2xl border-[12px] border-white group-hover:scale-105 transition-transform" />
                ...
                <div className="w-64 h-64 bg-slate-200 rounded-[4rem] flex items-center justify-center text-slate-400 text-8xl shadow-inner"><i className="fas fa-camera"></i></div>
              )}
              <input type="file" ref={fileInputRef} onChange={handleLogoUpload} accept="image/*" className="hidden" />
              <button onClick={() => fileInputRef.current?.click()} className="px-16 py-8 bg-onyx text-white rounded-[2.5rem] font-black uppercase text-lg tracking-[0.2em] hover:bg-ferrari transition-all flex items-center gap-6 shadow-2xl">
                <i className="fas fa-upload text-3xl"></i> ATUALIZAR LOGO
              </button>
            </div>
          </div>
        )}
      ...
      {isProductModalOpen && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/70 backdrop-blur-md animate-fade-in">
          <div className="bg-white w-full max-w-5xl rounded-[5rem] shadow-2xl overflow-hidden animate-fade-in-up border border-white/20">
            ...
            <form onSubmit={handleSaveProduct} className="p-20 space-y-12">
              <div className="flex justify-between items-center">
                <h3 className={`text-5xl ${textClass}`}>{editingProductId ? 'Editar Item' : 'Novo Item'}</h3>
                <button type="button" onClick={closeModal} className="text-gray-300 hover:text-ferrari transition-all hover:rotate-90"><i className="fas fa-times text-5xl"></i></button>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                <div className="space-y-4">
                  <label className={`text-base pl-6 ${textClass}`}>Categoria</label>
                  <select 
                    value={newProduct.category} 
                    onChange={e => setNewProduct({...newProduct, category: e.target.value as Category})}
                    className="w-full bg-slate-50 border-none rounded-[2.5rem] p-8 font-black uppercase text-lg outline-none shadow-inner h-24"
                  >
                ...
                <div className="space-y-4">
                  <label className={`text-base pl-6 ${textClass}`}>Nome do Item</label>
                  <input 
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
                <div className="space-y-4">
                  <label className={`text-base pl-6 ${textClass}`}>Preço Venda</label>
                  <input 
                    type="number" step="0.01" required value={newProduct.price}
                    onChange={e => setNewProduct({...newProduct, price: parseFloat(e.target.value)})}
                    className="w-full bg-slate-50 border-none rounded-[2.5rem] p-8 font-black text-lg outline-none shadow-inner h-24"
                  />
                </div>
                <div className="space-y-4">
                  <label className={`text-base pl-6 ${textClass}`}>Custo Unit.</label>
                  <input 
                ...
                <div className="space-y-4">
                  <label className={`text-base pl-6 ${textClass}`}>Estoque</label>
                  <input 
              ...
              <div className="space-y-8">
                <label className={`text-base pl-6 ${textClass}`}>Imagem do Produto</label>
                <div className="flex items-center gap-12">
                  <img src={newProduct.image} className="w-48 h-48 rounded-[3rem] object-cover shadow-2xl border-4 border-white" />
                  ...
                  <button 
                    ...
                    className="px-12 py-6 bg-slate-100 text-gray-500 rounded-3xl font-black uppercase text-sm tracking-widest hover:bg-slate-200 transition-all"
                  >
                    MUDAR FOTO
                  </button>
                </div>
              ...
              <button 
                type="submit"
                className="w-full bg-onyx text-white py-12 rounded-[3.5rem] font-black uppercase tracking-[0.4em] text-lg hover:bg-ferrari transition-all shadow-2xl shadow-onyx/30"
              >
